...
Before Psi archictures get explained, we define agents and environments according to Russel and Norvig.

An agent is defined as anything that can perceive its environment through sensors and conduct actions inside the environment via actuators. A software agent that may receive network packets as sensory inputs and outputs by writing files and sending network packets as acting on the environment serves as one example, which fits in nicely for this thesis.

They introduce the percept, as the agent's input and the percept sequence as the complete history of what the agent has perceived. Every choices an agent makes, are based on the percept sequence.

Furthermore, they describe the agent function, as the function that maps the percept sequence to an action. The agent program is its concrete implementation.

A rational agent is an agent that acts appropriately to any given percept sequence. To answer the question about what is appropriate and what is not, they introduce performance measures that evaluate the agents actions. These measures have to be defined by the designer of the experiment. Therefore, the rational agent tries to maximise its performance measure at any given point of time.

A task environment is the combination of the performance measure, the environment and the agent's actuators and sensors.

The environment have different properties. They can either be fully observable or partially observable. An environment is fully observable when the agent has access to the complete state of the environment or at least to to everything that is relevant regarding to the performance measure. As opposed to this, it is partially observable, if the beforementioned requirement is not given.

An environment can furtherermore be single agent or multiagent. An environment is multiagent, when there is at least one other agent, that's behaviour is about maximising its performance measure that depends on the first agents behaviour. If the agents try to maximise the performance measure of all agents, the environment is called cooperative. If agents try to maximise their own and minimise the performance measure of the other agents, it is called competitive.

An environment may furthermore be discrete or continuous. If an environment can be thought of as advancing in discrete state to the other (and even more so if there is a finite number of states).
